...
(function (window, document) {
  ...
  var Genki = {
          ...
          quiz += '<div id="quiz-q' + i + '" class="question-block" data-qid="' + (i + 1) + '" style="display:none;">'+
            ...
            '<div class="quiz-multi-question">'+
              ...
              '<div class="kanji-stroke-order">'+
                '<a class="button" href="' + (q[i].kana ? getPaths() + 'resources/images/stroke-order/sasagami-' + o.kanaType.toLowerCase() + '.jpg' : 'https://jisho.org/search/' + q[i].kanji + '%20%23kanji') + '" target="_blank" title="View stroke order ' + (q[i].kana ? 'chart' : 'on jisho.org') + '"><i class="fa">&#xf002;</i>View on jisho.org</a><br>'+
                '<div class="stroke-order-container"><img src="' + img + '" alt="stroke order"/></div>'+
              '</div>'+
            '</div>'+
          ...
          if (++Genki.dataBackupReminderCount >= 10) {
            ...
            setTimeout(function() {
              GenkiModal.open({
                ...
                '<div class="center">'+
                  '<a id="save-exercise-data" class="button" download="Tobira Exercise Score Data" href="data:,' + (storageOK && localStorage.TobiraResults ? encodeURIComponent(localStorage.TobiraResults.replace(/\n/g, '\r\n')) : '') + '"><i class="fa">&#xf019;</i>Save Data</a><br><br>'+
    ...
    breakTime : function () {
      GenkiModal.open({
        title : 'Take a Break?',
        content : 'Taking a break and waiting before trying again can greatly help with building your memory. 5 to 10 minute breaks are recommended, but you\'re free to adjust the time to your liking.' + (/"format":"vocab"/.test(Genki.exerciseData) ? ' Please see <a href="' + getPaths() + 'help/vocab-memorization/' + Genki.local + '" target="_blank">this page</a> for more tips on memorizing vocab.' : '') + '<br><br>'+
      ...
      exerciseList : function () {
        var main = 
          '<div id="link-list" class="normal-block indent-block">'+
            '<div><a id="link-home" class="button" href="' + (getPaths() == '' ? '#main-title' : getPaths() + Genki.local) + '"><i class="fa">' + (getPaths() == '' ? '&#xf062;' : '&#xf015;') + '</i>' + (getPaths() == '' ? 'Top' : 'Home') + '</a></div>'+
            '<div><a id="link-grammar" href="' + getPaths() + 'lessons/appendix/grammar-index/' + Genki.local + '"><i class="fa">&#xf02d;</i>Grammar Index</a></a></div>'+
        ...
        if (Genki.exercises) {
          var attrs = 'class="lesson-title" onclick="Genki.toggle.list(this);" onkeydown="event.key == \'Enter\' && Genki.toggle.list(this);" tabindex="0"', // lesson-title attrs
              ...
              '<nav id="exercise-list">'+
                ...
                '<div id="lessons-list"><h4 ' + attrs + '>ページリンク</h4><ul id="page-links">' + main + '</ul>',
              lesson = '\\.\\.\\/',
              i = 0,
      ...
      GenkiModal.open({
        ...
        content :
          '<div class="kanji-stroke-order center">'+
            ...
            '<a class="button" href="' + (kana ? getPaths() + 'resources/images/stroke-order/sasagami-' + kana + '.jpg' : 'https://jisho.org/search/' + kanji + '%20%23kanji') + '" target="_blank" title="View stroke order ' + (kana ? 'chart' : 'on jisho.org') + '"><i class="fa">&#xf002;</i>View on jisho.org</a><br>'+
            '<div class="stroke-order-container"><img src="' + img + '" alt="stroke order"/></div>'+
          '</div>'
